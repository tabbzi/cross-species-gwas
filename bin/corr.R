...
dir_path <- "~/Dropbox (UMass Medical School)/Papers/cross-species/git/cross-species-gwas/dat/gwa/dog/bat/"

# Get a list of files in the directory
file_list <- list.files(path = dir_path, full.names = TRUE)

# Read in all files in the list using map_dfr()
data_list <- map_dfr(file_list, read_tsv)
...
test.csq.corr = comp %>%
  group_by(dog.phe) %>%
  do(cor_test = cor.test(.$dog.chisq, 
                         .$hum.chisq) %>% tidy()) %>%
  unnest(cor_test)

test.csq.kendall.corr = comp %>%
  group_by(dog.phe) %>%
  do(cor_test = cor.test(.$dog.chisq, 
                         .$hum.chisq,
                         method = "kendall") %>% tidy()) %>%
...
test.p.kendall.corr %>% 
  filter(dog.phe %like% "fa.") %>% 
  mutate(p.adj = p.adjust(p = p.value, method = "BH")) %>%
  ggplot(aes(x = dog.phe, 
             y = estimate, 
             fill = p.adj < 0.05)) + 
  coord_flip() +
  geom_col() +
